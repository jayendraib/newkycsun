...
# ── TEST MODE FLAG ─────────────────────────────────────────────────────────────
# Set TEST_MODE=true in your .env or leave it unset to default to True.
# When True: auth-id header is optional (any value works), and webhooks hit local endpoints.
# When False: prod auth + prod webhooks are enforced.
...
# ── PROD WEBHOOKS — COMMENTED OUT FOR TESTING ─────────────────────────────────
...
# ── LOCAL TEST WEBHOOKS — USE THESE INSTEAD ───────────────────────────────────
# These point back to this same FastAPI app so you can see webhook payloads in logs.
WEBHOOK_URL = os.getenv("SUMMARY_WEBHOOK_URL", "http://localhost:8000/test-webhook/summary")
WEBHOOK_TOKEN = os.getenv("SUMMARY_WEBHOOK_TOKEN", "test-token-local")
KYC_AUTH_ID = os.getenv("KYC_AUTH_ID", "test-auth-local")
KYC_WEBHOOK_URL = os.getenv("KYC_WEBHOOK_URL", "http://localhost:8000/test-webhook/kyc")
...
def kyc_handler(data: dict) -> dict:
    ...
    try:
        if zip_path.startswith("s3://"):
            ...
            # LOCAL PATH: works for absolute paths like /home/user/zips/file.zip
...
class KycRequest(BaseModel):
    ...
    zip_s3_path: str        # full S3 path OR local absolute path, e.g. "/home/user/zips/file.zip"


class SummaryRequest(BaseModel):
    url: str                # PDF URL to download and summarise (can be local file:// or http://)
    news_id: int            # news record ID — echoed back in the webhook payload


# ── LOCAL TEST WEBHOOK RECEIVERS ──────────────────────────────────────────────
# These just log what they receive so you can verify webhooks fire without hitting prod.

@app.post("/test-webhook/kyc")
async def test_kyc_webhook(request: Request):
    body = await request.json()
    logger.info(f"🧪 TEST KYC WEBHOOK RECEIVED: {body}")
    return {"status": "test_received", "payload": body}


@app.post("/test-webhook/summary")
async def test_summary_webhook(request: Request):
    body = await request.json()
    logger.info(f"🧪 TEST SUMMARY WEBHOOK RECEIVED: {body}")
    return {"status": "test_received", "payload": body}
...
def submit_kyc(req: KycRequest, auth_id: str | None = Header(None, alias="auth-id")):
    # TEST_MODE: skip auth check, accept any or no auth-id
